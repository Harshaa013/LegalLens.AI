...
export const CompareView: React.FC<CompareViewProps> = ({ contracts, onBack }) => {
      ...
      return (
          <div className="flex flex-col items-center justify-center min-h-[60vh] animate-fade-in">
              <div className="relative mb-8">
                ...
                <div className="relative z-10 bg-white p-6 rounded-2xl shadow-lg border border-slate-100">
                   <Scale className="w-12 h-12 text-indigo-600 animate-pulse" />
                </div>
              </div>
              <h2 className="text-2xl font-bold text-slate-800 mb-2">Comparing Contracts</h2>
  ...
  return (
    <div className="space-y-8 animate-slide-up relative">
      <button 
        onClick={onBack}
        className="flex items-center text-slate-500 hover:text-indigo-600 transition-colors font-medium hover:-translate-x-1 duration-200"
      >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-3 bg-gradient-to-r from-indigo-600 to-indigo-700 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden">
            <div className="absolute top-0 right-0 p-12 opacity-10 transform translate-x-10 -translate-y-10">
                <Trophy className="w-64 h-64 text-white" />
            </div>
            
            <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4 text-indigo-200 font-semibold uppercase tracking-wider text-sm">
        ...
        <div className="lg:col-span-3 relative">
            ...
            {contracts.length === 2 && (
                <div className="absolute left-1/2 top-8 -translate-x-1/2 z-10 hidden md:flex items-center justify-center w-12 h-12 bg-white rounded-full shadow-lg border-4 border-slate-50 font-black text-slate-300">
                    VS
                </div>
            )}
                ...
                {contracts.map(contract => (
                    <div 
                        ...
                        className={`bg-white rounded-2xl p-6 border-2 transition-all ${
                            ...
                            ? 'border-emerald-400 shadow-lg ring-4 ring-emerald-50' 
                            : 'border-slate-200'
                        }`}
                    >
                        <div className="flex justify-between items-start mb-6">
                            <h3 className="text-xl font-bold text-slate-900 truncate pr-4" title={contract.fileName}>{contract.fileName}</h3>
                            {comparison.recommendedId === contract.id && (
                                <span className="bg-emerald-100 text-emerald-700 text-xs font-bold px-3 py-1 rounded-full flex items-center shrink-0">
                        ...
                        <div className="space-y-6">
                            <div>
                                <div className="text-xs font-semibold text-slate-400 uppercase mb-1">Risk Score</div>
                                <div className="flex items-end gap-2">
                                    <span className="text-4xl font-bold text-slate-800">{contract.analysis?.riskScore}</span>
                            ...
                            <div>
                                <div className="text-xs font-semibold text-slate-400 uppercase mb-2">Overall Risk</div>
                                {contract.analysis && <RiskBadge level={contract.analysis.overallRisk} />}
                            </div>

                            <div>
                                <div className="text-xs font-semibold text-slate-400 uppercase mb-2">Summary</div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    {comparison.keyDifferences.map((diff, idx) => (
                        <button 
                            ...
                            className="flex items-start text-left p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-indigo-300 hover:shadow-md hover:bg-white transition-all group"
                        >
                            <div className="bg-white p-2 rounded-full border border-slate-200 group-hover:border-indigo-200 mr-3 shrink-0 mt-0.5 shadow-sm">
                                <MessageCircle className="w-4 h-4 text-indigo-500" />
                            </div>
                            <p className="text-slate-700 text-sm leading-relaxed group-hover:text-slate-900">{diff}</p>
                        </button>
                    ))}
                </div>
            </div>
...
const DifferenceChatModal: React.FC<DifferenceChatModalProps> = ({ difference, contracts, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col relative z-10 animate-scale-in border border-white/50">
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-start bg-slate-50/50 rounded-t-2xl">
                    <div>
                        ...
                        <h3 className="text-lg font-bold text-slate-900 leading-snug">{difference}</h3>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-5 bg-white">
                    ...
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            {msg.role === 'model' && (
                                <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center mr-3 mt-1 shrink-0">
                            ...
                            {msg.role === 'user' && (
                                <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center ml-3 mt-1 shrink-0">
                    ...
                    {isLoading && messages.length > 0 && (
                        <div className="flex justify-start">
                            <div className="w-8 h-8 bg-indigo-100 rounded-full flex items-center justify-center mr-3 shrink-0">
                                <Bot className="w-5 h-5 text-indigo-600" />
                            </div>
                            <div className="bg-slate-50 border border-slate-100 rounded-2xl rounded-tl-none px-5 py-4 shadow-sm flex space-x-1.5 items-center h-12">
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
                    <div className="flex items-center gap-3">
                        <input
                            ...
                            className="flex-1 px-4 py-3 bg-white text-slate-900 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition-all"
                            autoFocus
                        />
                        <button
                            onClick={handleSend}
